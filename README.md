...
### Takeaways
From this project, we've learned so much more about how nodes work and how to publish and subscribe to topics. But even bigger than that is just learning how sensors work. Before coming into comp robo, we had no idea that sensors didn't all just run on the same time-frame or that sensors could publish data to multiple places at the same time. On top of that, another big takeaway is that mapping out your environment is super important. For our draw shape behavior, we were publishing data to /cmd_vel to draw a star, but due to factors like friction it didn't actually draw a star. It was interesting to see that although the neato thought it was doing something, it wasn't really doing that thing. The only way to confirm that is by making sense of the environment. We find this especially cool because this makes robotics more than telling a robot to do something, but robotics also involves understanding the surroundings your designing for!